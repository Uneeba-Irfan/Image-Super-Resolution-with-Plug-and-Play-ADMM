...
dataset = CustomImageDataset(root_dir='images', image_size=(1920, 1440))
dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)

model = CustomDenoiser().to(device)
if os.path.isfile("custom_denoiser_noise_0.05.pt"):
    model.load_state_dict(torch.load("custom_denoiser_noise_0.05.pt"))
...
for epoch in range(num_epochs):
    ...
    torch.save(model.state_dict(), 'custom_denoiser_noise_0.05.pt')
