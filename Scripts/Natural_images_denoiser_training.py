...
class CustomNaturalImageDataset(Dataset):
    def __init__(self, root_dir, image_size, noise_level):
    ...
    def __getitem__(self, idx):
        ...
        clean_image, noisy_image = image_prep_denoising(image, self.noise_level)
...
for noise in noise_levels:
    ...
    dataset = CustomNaturalImageDataset(root_dir='natural_images', image_size=(1920, 1440), noise_level=noise)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
    
    model = CustomDenoiser().to(device)
    model_file = f"custom_denoiser_natural_imgs_noise_{noise:.4f}.pt"
